...
export default function ProductCheckout({ title, description, price, imageUrl }: ProductCheckoutProps) {
  return (
    <div className="w-full px-7.5 py-5 flex flex-row gap-7.75 bg-[#2B2B2B] rounded-lg">
      ...
      <div className="flex">
        <div className="w-40.25 h-40.25 flex items-center justify-center bg-slate-custom rounded-lg">
          <Image src={imageUrl} alt={title} width={139} height={139} />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 justify-center min-w-0">
        ...
        <div className="w-full flex flex-row justify-between">
          ...
          <div className="flex flex-row bg-background rounded-lg max-w-28.75">
            <button className="px-2 py-4 cursor-pointer group">
              <Icons.Minus className="group-hover:text-primary" />
            </button>
            <input type="text" className="ring-0 outline-none w-full text-center" />
            <button className="px-2 py-4 cursor-pointer group">
              <Icons.Plus className="group-hover:text-primary" />
            </button>
          </div>

          <button className="w-10.75 h-10.75 bg-primary rounded-full flex items-center justify-center" >
